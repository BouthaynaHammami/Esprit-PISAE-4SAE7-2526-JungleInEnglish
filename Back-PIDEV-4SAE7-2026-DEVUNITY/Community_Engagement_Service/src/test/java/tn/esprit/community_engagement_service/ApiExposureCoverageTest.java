package tn.esprit.community_engagement_service;
...
class ApiExposureCoverageTest {
    ...
    private static final String BASE_PACKAGE = "tn.esprit.community_engagement_service";

    @Test
    void shouldExposeApiMappingsAcrossEventsModules() {
        ...
        long crudLikeCount = endpoints.stream()
                .filter(e -> e.path.toLowerCase().matches(".*(add|create|update|delete|all|id|event|registration|kanban|ticket).*")
