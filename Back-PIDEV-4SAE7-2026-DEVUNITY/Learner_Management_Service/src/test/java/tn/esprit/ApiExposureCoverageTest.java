...
class ApiExposureCoverageTest {
    ...
    private static final String BASE_PACKAGE = "tn.esprit";

    @Test
    void shouldExposeApiMappingsAcrossUserManagementLevelTestAndBooksClubsModules() {
        List<Endpoint> endpoints = discoverEndpoints(BASE_PACKAGE);
        assertFalse(endpoints.isEmpty(), "No API endpoints discovered");
        assertTrue(endpoints.size() >= 50, "Expected at least 50 endpoints, found " + endpoints.size());

        long crudLikeCount = endpoints.stream()
                .filter(e -> e.path.toLowerCase().matches(".*(add|create|update|delete|all|id|book|club|wallet|order|rental|membership|subject|test|auth|user).*")
    ...
    private List<Endpoint> discoverEndpoints(String basePackage) {
        ...
        scanner.findCandidateComponents(basePackage).forEach(bean -> {
            try {
                ...
                if (!controllerClass.getName().contains("Controllers") && !controllerClass.getName().contains("controllers") && !controllerClass.getName().contains("AuthController")) {
                    return;
                }
