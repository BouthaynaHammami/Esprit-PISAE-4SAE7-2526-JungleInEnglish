...
class ApiExposureCoverageTest {
    ...
    private static final String BASE_PACKAGE = "tn.esprit.language_courses_service";

    @Test
    void shouldExposeApiMappingsAcrossBusinessEnglishAndChildrenEnglishModules() {
        List<Endpoint> endpoints = discoverEndpoints(BASE_PACKAGE);
        assertFalse(endpoints.isEmpty(), "No API endpoints discovered");
        assertTrue(endpoints.size() >= 30, "Expected at least 30 endpoints, found " + endpoints.size());

        long crudLikeCount = endpoints.stream()
                .filter(e -> e.path.toLowerCase().matches(".*(add|create|update|delete|all|id|offer|invitation|progress|course|child|parent|reward).*")
