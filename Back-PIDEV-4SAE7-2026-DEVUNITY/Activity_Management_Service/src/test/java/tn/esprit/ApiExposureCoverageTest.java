...
class ApiExposureCoverageTest {
    ...
    @Test
    void shouldExposeApiMappingsAcrossEmployeeSchedulesAndChallengesModules() {
        List<Endpoint> endpoints = discoverEndpoints(BASE_PACKAGE);
        assertFalse(endpoints.isEmpty(), "No API endpoints discovered");
        assertTrue(endpoints.size() >= 40, "Expected at least 40 endpoints, found " + endpoints.size());

        long crudLikeCount = endpoints.stream()
                .filter(e -> e.path.toLowerCase().matches(".*(add|create|update|delete|all|id|room|class|schedule|recruitment|interview|applicant|challenge|badge).*")
