...
@Component({
  selector: 'app-admin-daily-analysis',
  templateUrl: './admin-daily-analysis.component.html',
  styleUrls: ['./admin-daily-analysis.component.scss']
})
export class AdminDailyAnalysisComponent implements OnInit {
  ...
  get totalTasks(): number {
    return (this.certifAnalysis?.totalTasks ?? 0) + (this.eventsAnalysis?.totalTasks ?? 0);
  }

  get totalTodo(): number {
    return (this.certifAnalysis?.todoCount ?? 0) + (this.eventsAnalysis?.todoCount ?? 0);
  }

  get totalDoing(): number {
    return (this.certifAnalysis?.doingCount ?? 0) + (this.eventsAnalysis?.doingCount ?? 0);
  }

  get totalDone(): number {
    return (this.certifAnalysis?.doneCount ?? 0) + (this.eventsAnalysis?.doneCount ?? 0);
  }

  get totalOverdue(): number {
    return (this.certifAnalysis?.overdueCount ?? 0) + (this.eventsAnalysis?.overdueCount ?? 0);
  }
